...
public class SequenceParser extends AbstractFieldParser {
    ...
    protected Field parse(Element sequenceElement, boolean optional, ParsingContext context) {
        Sequence sequence = Fast.SIMPLE.createSequence(context.getName(), parseSequenceLengthField(context.getName(), sequenceElement, optional,
                context), GroupParser.parseFields(sequenceElement, context));
        GroupParser.parseMore(sequenceElement, sequence, context);
        return Fast.SIMPLE.createField(sequence, optional);
    }
    ...
    private Scalar parseSequenceLengthField(QName name, Element sequence, boolean optional, ParsingContext parent) {
        ...
        if (lengthElements.getLength() == 0) {
            Scalar implicitLength = new Scalar(Global.createImplicitName(name), FastTypes.U32, null, optional);
